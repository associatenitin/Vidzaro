  ...
  return (
    <div className="bg-slate-800 border-b border-slate-700 px-6 py-3 flex items-center gap-4">
      ...
      <div className="relative">
        ...
        {prefsOpen && (
          <>
            <div className="fixed inset-0 z-40" onClick={() => setPrefsOpen(false)} aria-hidden="true" />
            <div className="absolute right-0 top-full mt-1 z-50 w-72 py-2 px-3 bg-slate-800 border border-slate-600 rounded-lg shadow-xl">
              <div className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Preferences</div>
              ...
              <p className="text-xs text-slate-500 mt-1.5">When off, uses CPU only (slower; no CUDA DLL errors).</p>
            </div>
          </>
        )}
      </div>
