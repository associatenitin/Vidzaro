...
export default function Toolbar({ onSplit, currentTime, isPlaying, onPlayPause, activeTool, onToolChange, onUndo, onRedo, canUndo, canRedo, onOpenPreferences, onAIEnhance, onGenAI, genAIProgress, onStartRecording, onMotionTracking, onVideoMorph, onCaptions }) {
  ...
  return (
    <div className="bg-slate-800 border-b border-slate-700 px-6 py-3 flex items-center gap-4 relative z-10 overflow-x-auto custom-scrollbar">
      ...
      {onCaptions && (
        <button
          onClick={onCaptions}
          className="px-4 py-2 min-h-[52px] bg-teal-600 hover:bg-teal-700 rounded-lg font-medium flex items-center gap-2 flex-shrink-0"
          title="Auto Captions - Generate speech-to-text captions for the selected clip"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h4m-7 8V6a2 2 0 012-2h12a2 2 0 012 2v9a2 2 0 01-2 2H9l-4 4z" />
          </svg>
          Captions
        </button>
      )}
      ...
      <div className="relative flex-shrink-0">
        <button
          ...
          className="px-4 py-2 min-h-[52px] rounded-xl font-medium flex items-center gap-2
            bg-gradient-to-br from-cyan-500/25 via-teal-500/20 to-emerald-500/20
            hover:from-cyan-500/35 hover:via-teal-500/30 hover:to-emerald-500/25
            backdrop-blur-md
            border border-cyan-400/30 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.2)]
            transition-all duration-200 hover:border-cyan-300/40 hover:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.25),0_0_20px_rgba(34,211,238,0.15)]"
          title="Browse royalty-free images and music (Pixabay)"
        >
          <span className="text-center leading-tight text-white drop-shadow-sm">Royalty Free<br />Images/Sound</span>
          <svg className="w-4 h-4 flex-shrink-0 text-cyan-100" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 9l6 6 6-6" />
          </svg>
        </button>

        {isPixabayOpen && (
          <div className="absolute mt-1 right-0 w-64 py-1 z-20
            bg-gradient-to-b from-cyan-950/40 to-teal-950/50 backdrop-blur-xl
            border border-cyan-400/25 rounded-xl
            shadow-[0_8px_32px_rgba(0,0,0,0.35),inset_0_1px_0_0_rgba(255,255,255,0.08),0_0_24px_rgba(34,211,238,0.08)]">
            <button
              type="button"
              onClick={() => handleOpenPixabay('https://pixabay.com/photos/')}
              className="w-full text-left px-3 py-2 text-sm text-slate-100 hover:bg-cyan-500/20 rounded-lg mx-1 transition-colors"
            >
              Royalty Free Images
            </button>
            <button
